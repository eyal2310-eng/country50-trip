...
`assets/rtl_docx.py` פותר את הבעיה הראשונה — הנפוצה ביותר — מעצמו:
`protect_bidi()` מוסיף תו RLM (U+200F) לפני סימן פיסוק סוגר שקודמת לו ספרה,
סוגר או תו לועזי. הפונקציה רצה אוטומטית על כל טקסט שעובר דרך
`para` / `bullet` / `heading` / `table`, כך ש-"ביום 3.2.2026." מוגן בלי לשנות
את הניסוח, ובלי שתצטרך לכתוב "ביום 3.2.2026 כאמור.".

זו הסיבה שאין לכתוב קוד python-docx מאפס: הכתיבה הישירה מאבדת את ההגנה הזו.

`assets/check_hebrew.py` מאמת את התוצאה ונכשל על כל סיום לא מוגן. הרץ אותו
על כל מסמך לפני מסירה.

לשאר המקרים בטבלה (טקסט לועזי באמצע משפט, טווחי מספרים) עדיין נדרש טיפול
ידני — run או span נפרד.
